...
public class RailEdgeRenderer{

    private Map<RailEdge<MCPos>, EdgeRenderer> edgesToRenderer = new HashMap<>();

    private EdgeRenderer getSectionRenderer(RailEdge<MCPos> edge){
        EdgeRenderer renderer = edgesToRenderer.get(edge);
        if(renderer == null) {
            renderer = new EdgeRenderer(edge);

            Set<Integer> invalidColors = getAdjacentEdges(edge).map(x -> x.colorIndex).collect(Collectors.toSet());
            ...
            if(availableColors > 0) { //If there are colors left (it would be very exceptional if there weren't.
                int usedIndex = Math.abs(edge.iterator().next().hashCode()) % availableColors; //Use a deterministic way to generate a color index.
                for(int i = 0; i < 16; i++) {
            ...
            edgesToRenderer.put(edge, renderer);
        }
        return renderer;
    }

    private Stream<EdgeRenderer> getAdjacentEdges(RailEdge<MCPos> edge){
        return edgesToRenderer.entrySet().stream().filter(e -> e.getKey().contains(edge.startPos) || e.getKey().contains(edge.endPos)).map(e -> e.getValue());
    }

    public void updateSectionRenderers(){
        Iterable<RailEdge<MCPos>> allEdges = RailNetworkManager.getInstance().getNetwork().getAllEdges();
        edgesToRenderer.clear();

        for(RailEdge<MCPos> edge : allEdges) {
            getSectionRenderer(edge);
        }
    }
    ...
    public void render(BufferBuilder b){
        ...
        for(EdgeRenderer edgeRenderer : edgesToRenderer.values()) {
    ...
    private static class EdgeRenderer{

        public int colorIndex;
        public RectRenderer rectRenderer;
        private final RailEdge<MCPos> edge;

        public EdgeRenderer(RailEdge<MCPos> edge){
            this.edge = edge;
        }
        ...
        public void compileRender(){
            ...
            NetworkRail<MCPos> rootNode = edge.iterator().next();
            ...
            while(!toTraverse.isEmpty()) {
                ...
                List<NetworkRail<MCPos>> neighbors = node.getSectionNeighborRails(edge.railObjects).collect(Collectors.toList());
